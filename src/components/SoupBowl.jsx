 ...
 * soup pours in and broccoli florets float on top.
 *
 * Props:
...
export default function SoupBowl({ filled, onClick, disabled }) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      aria-label={filled ? 'Bowl with broccoli cheddar soup' : 'Empty red bowl, click to check'}
    >
      <svg viewBox="0 0 320 240" xmlns="http://www.w3.org/2000/svg">
        <defs>
          {/* Clip the soup to the bowl interior */}
          <clipPath id="bowl-interior">
            <path d="M 50 110 Q 50 200 160 200 Q 270 200 270 110 Z" />
          </clipPath>

          {/* Soup gradient — broccoli cheddar orange/yellow */}
        ...
        {/* Steam wisps (only visible when filled) */}
        <g className="steam">
        ...
        {/* Soup — clipped to bowl interior */}
        <g clipPath="url(#bowl-interior)" className="soup-layer">
          <rect className="soup-fill" x="50" y="100" width="220" height="120" fill="url(#soup-gradient)" />

          {/* Soup surface highlight */}
          <ellipse className="soup-surface" cx="160" cy="108" rx="108" ry="10" fill="#f9c84d" opacity="0.6" />

          {/* Broccoli florets floating on top */}
          ...
          {/* Cheese specks */}
          <circle className="cheese" cx="130" cy="115" r="2" fill="#fff3a0" />
          <circle className="cheese" cx="180" cy="113" r="2.5" fill="#fff3a0" />
          <circle className="cheese" cx="195" cy="116" r="1.8" fill="#fff3a0" />
          <circle className="cheese" cx="140" cy="117" r="1.5" fill="#fff3a0" />
        </g>
